		...
		<view class="table-all-box">
			<scroll-view scroll-x="true" class="table-all">
				...
				<view class="table-row" v-for="(item, index) in dataSource" :key="index">
					...
					<view class="table-td">{{item.classCampusId || ""}}</view>
	...
	export default {
		...
		methods: {
			...
			handleScoreValue(item) {
				const sportItem = item.sportItems[0];
				const dianaScoreConfig = this.schoolInfo.dianaScoreConfig;
				return configSportScore(item.score, sportItem.sportItemCode, sportItem.value, dianaScoreConfig);
			},
			// 处理成绩状态
			...
			handleTrainTaskScore(item) {
				...
				return MathUtil.computeNumber(item.trainTaskScore, '/', 100).result
			},
