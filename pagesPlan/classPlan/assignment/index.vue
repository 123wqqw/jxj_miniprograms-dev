<template>
  <view class="basic-container">
    ...
    <u-calendar
      ...
      max-date="2030-12-31"
      :min-date="$u.timeFormat(new Date(), 'yyyy-mm-dd')"
      @change="bindCalendarChange"
    ></u-calendar>
    <u-modal
      v-model="modalShow"
      :show-cancel-button="true"
      @cancel="modalShow = false"
      @confirm="confirm(value ? true : false)"
      ref="uModal"
      :async-close="true"
    >
...
export default {
  ...
  methods: {
    async getClassInfo() {
      try {
        const result = await getReq(URL.apiGetPhysicalClassReportTop, {
          ...this.urlOption,
        });
    ...
    async confirm(replace) {
      try {
        ...
        const params = {
          assignDates: this.getWeek(
            this.dateRange[0],
            this.dateRange[1],
            weekNum
          ),
          classIds: [this.urlOption.classId],
          replace,
          coursewareId: this.urlOption.id,
        };
        const result = await postReq(URL.apiPostCourseAssign, {
          ...params,
        });
    ...
    handleSumbit() {
      if (this.dateRange && this.dateRange.length) {
        postReq(URL.taskPublishCalendar, {
