	...
	export default {
		...
		methods: {
			...
			init() {
				...
				if(!this.canToday){
					let nextDayTime = now.getTime() + 24*60*60*1000;
					let nextDate = new Date();
					nextDate.setTime(nextDayTime);
					let year = nextDate.getFullYear();
					let month = nextDate.getMonth() + 1;
					let day = nextDate.getDate();
					let minChooseDate = `${year}-${month}-${day}`;
					console.log(minChooseDate);
					this.min = this.initDate(minChooseDate);
					this.activeDate = minChooseDate;
				}else{
					this.min = this.initDate(this.today);
				}
				if(this.maxDate){
					this.max = this.initDate(this.maxDate);
				}else{
					let nextMonthTime = now.getTime() + 30*24*60*60*1000;
					let nextDate = new Date();
					nextDate.setTime(nextMonthTime);
					let year = nextDate.getFullYear();
					let month = nextDate.getMonth() + 1;
					let day = nextDate.getDate();
					let maxChooseDate = `${year}-${month}-${day}`;
					this.max = this.initDate(maxChooseDate);
				}
				this.startDate = "";
				this.startYear = 0;
				this.startMonth = 0;
			...
			changeData() {
				...
				this.$emit('change', {
					type: 'range',
					detail: `${ this.year }-${ this.getMonthStrict(this.month) }`,
					// isToday: false,
				})
