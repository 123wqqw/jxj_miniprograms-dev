				...
				<view class="right-content">
					...
					<view class="filter-section">
						<view class="filter-item gray" @tap="toggleDifficultyDropdown">
							...
							<image src="/static/images/common/arrow-down-yes.png" class="filter-arrow" :class="{ rotated: showDifficultyDropdown }"></image>
						</view>
						<view class="filter-item gray" @tap="toggleDirectionDropdown">
							<text class="filter-text">{{ selectedDirection }}</text>
							<image src="/static/images/common/arrow-down-yes.png" class="filter-arrow" :class="{ rotated: showDirectionDropdown }"></image>
						</view>
					</view>
					<!-- 下拉面板：难度 -->
					<view v-if="showDifficultyDropdown" class="dropdown-panel">
					  <view class="option-item" v-for="opt in difficultyOptions" :key="opt.value" @tap="chooseDifficulty(opt)">
					    <text class="option-text" :class="{ selected: selectedDifficulty === opt.label }">{{ opt.label }}</text>
					  </view>
					</view>
					<!-- 下拉面板：改善方向 -->
					<view v-if="showDirectionDropdown" class="dropdown-panel">
					  <view class="option-item" v-for="opt in directionOptions" :key="opt.value" @tap="chooseDirection(opt)">
					    <text class="option-text" :class="{ selected: selectedDirection === opt.label }">{{ opt.label }}</text>
					  </view>
					</view>
					<!-- 下拉遮罩 -->
					<view v-if="showDifficultyDropdown || showDirectionDropdown" class="dropdown-mask" @tap="closeDropdowns"></view>
					<!-- 运动项目网格 -->
